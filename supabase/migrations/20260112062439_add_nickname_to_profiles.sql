-- =====================================================================================
-- Add nickname feature to profiles table
-- =====================================================================================

-- 1) Add nickname column to profiles table
ALTER TABLE public.profiles ADD COLUMN nickname TEXT;

-- 2) Set default nickname to be first_name + last_name for existing users
UPDATE public.profiles
SET nickname = TRIM(COALESCE(first_name, '') || ' ' || COALESCE(last_name, ''))
WHERE nickname IS NULL AND (first_name IS NOT NULL OR last_name IS NOT NULL);

-- 3) Update get_profile_names to return nickname
...
-- 4) Update get_public_products to use nickname with fallback
...
-- 5) Update get_public_product_detail to use nickname with fallback
...
-- 6) Update get_user_conversations to use nickname with fallback
...
-- 7) Update get_conversation_details to use nickname with fallback
...
-- 8) Update get_user_saved_products to use nickname with fallback
...
-- 9) Update get_message_read_receipts to use nickname with fallback
